# Zote Precepts

A solemn memorial for a wise grey prince

## Installation

This repository requires git-lfs before cloning

1. Install git-lfs (https://git-lfs.com/)
2. Run `git lfs install`
3. You're OK to clone

Once the repo is cloned, run the project using

```sh
npm install
npm run dev
```

# License

Background music is an edited version of https://pixabay.com/fr/music/m%c3%a9ditation-spirituel-piano-and-strings-meditation-168763/

Font (Cormorant) used with the Open Font License
